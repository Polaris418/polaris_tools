...
public class AuthServiceImpl implements AuthService {
    ...
    private final com.polaris.email.service.EmailService emailService;
    ...
    private User findUserByEmailConsideringNormalization(String email) {
        if (email == null || email.isEmpty()) {
            return null;
        }
    ...
    private boolean isEmailRegistered(String email) {
        return findUserByEmailConsideringNormalization(email) != null;
    ...
    public UserResponse register(UserRegisterRequest request) {
        ...
        // 验证邮箱唯一性
        existingUser = findUserByEmailConsideringNormalization(request.getEmail());
        if (existingUser != null) {
    ...
    public UserResponse updateProfile(UserUpdateRequest request) {
        ...
        if (request.getEmail() != null && !normalizeEmail(request.getEmail()).equals(normalizeEmail(user.getEmail()))) {
            String normalizedEmail = normalizeEmail(request.getEmail());
            // 验证邮箱唯一性（排除当前用户）
            User existingUser = findUserByEmailConsideringNormalization(request.getEmail());
            ...
            log.info("用户邮箱已修改，标记为未验证: userId={}, oldEmail={}, newEmail={}", 
                    userId, oldEmail, normalizedEmail);
            
            // 异步发送验证邮件到新邮箱
            sendEmailVerificationAsync(userId, normalizedEmail, user.getUsername());
        }
    ...
    public void updateEmail(String newEmail, String password) {
        ...
        // 验证新邮箱唯一性
        User existingUser = findUserByEmailConsideringNormalization(newEmail);
        if (existingUser != null && !existingUser.getId().equals(userId)) {
        ...
        log.info("邮箱修改成功，已标记为未验证: userId={}, newEmail={}", userId, normalizedNewEmail);
        
        // 异步发送验证邮件到新邮箱
        sendEmailVerificationAsync(userId, normalizedNewEmail, user.getUsername());
    }
    ...
    public SendVerificationCodeResponse sendRegisterCode(SendVerificationCodeRequest request) {
        ...
        // 2. 检查邮箱是否已注册
        User existingUser = findUserByEmailConsideringNormalization(email);
        if (existingUser != null) {
        ...
        if (response == null || !Boolean.TRUE.equals(response.getSuccess())) {
            String errorMessage = response != null && response.getMessage() != null
                    ? response.getMessage()
                    : "验证码发送失败，请稍后重试";
            recordVerificationLog(email, purpose, "send", ipAddress, false, errorMessage);
            log.warn("注册验证码发送失败: email={}, error={}", email, errorMessage);
            throw new BusinessException(ErrorCode.EMAIL_SEND_FAILED, errorMessage);
        }
    ...
    public SendVerificationCodeResponse sendLoginCode(SendVerificationCodeRequest request) {
        ...
        if (response == null || !Boolean.TRUE.equals(response.getSuccess())) {
            String errorMessage = response != null && response.getMessage() != null
                    ...
                    : "验证码发送失败，请稍后重试";
            recordVerificationLog(email, purpose, "send", ipAddress, false, errorMessage);
            log.warn("登录验证码发送失败: email={}, error={}", email, errorMessage);
            throw new BusinessException(ErrorCode.EMAIL_SEND_FAILED, errorMessage);
        }
        recordVerificationLog(email, purpose, "send", ipAddress, true, null);
    ...
    public SendVerificationCodeResponse sendResetPasswordCode(SendVerificationCodeRequest request) {
        ...
        if (response == null || !Boolean.TRUE.equals(response.getSuccess())) {
            String errorMessage = response != null && response.getMessage() != null
                    ...
                    : "验证码发送失败，请稍后重试";
            recordVerificationLog(email, purpose, "send", ipAddress, false, errorMessage);
            log.warn("密码重置验证码发送失败: email={}, error={}", email, errorMessage);
            throw new BusinessException(ErrorCode.EMAIL_SEND_FAILED, errorMessage);
        }
        recordVerificationLog(email, purpose, "send", ipAddress, true, null);
    ...
    public SendVerificationCodeResponse sendChangePasswordCode(SendVerificationCodeRequest request) {
        ...
        if (response == null || !Boolean.TRUE.equals(response.getSuccess())) {
            String errorMessage = response != null && response.getMessage() != null
                    ...
                    : "验证码发送失败，请稍后重试";
            recordVerificationLog(email, purpose, "send", ipAddress, false, errorMessage);
            log.warn("修改密码验证码发送失败: email={}, error={}", email, errorMessage);
            throw new BusinessException(ErrorCode.EMAIL_SEND_FAILED, errorMessage);
        }
        recordVerificationLog(email, purpose, "send", ipAddress, true, null);
